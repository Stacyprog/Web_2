...
public class AreaCheckServlet extends HttpServlet {
    ...
    private final static List<String> AVALIABLE_X = Arrays.asList("-2", "-1.5", "-1", "-0.5", "0", "0.5", "1", "1.5", "2");
    private final static List<String> AVALIABLE_R = Arrays.asList("1", "2", "3", "4", "5");
    ...
    private HistoryQuery doCheck(double x, double y, int r, long startingTimeNanos, HistoryBean history) {
        boolean result = (x <= 0 && y <= 0 && y >= -x / 2. - r / 2.) // First quarter
                ...
                || (x <= 0 && y >= 0 && x >= -r && y <= r / 2.); // Third quarter
    ...
    static Double parseX(String str, boolean fromCanvas) {
        try {
            if (AVALIABLE_X.contains(str) || fromCanvas) {
